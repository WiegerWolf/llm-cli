...
std::string perform_deep_research(Database& db, ChatClient& client, UserInterface& ui, const std::string& goal) {
    ...
    try {
        ...
        std::vector<app::db::Message> subquery_context;
        ...
        try {
            ...
            // JSON Parsing Error (Sub-query Generation)
        ...
        if (!subquery_response_json.contains("choices") || subquery_response_json["choices"].empty() || !subquery_response_json["choices"][0].contains("message") || !subquery_response_json["choices"][0]["message"].contains("content")) {
            // Error: Invalid API response structure (Sub-query Generation).
        ...
        try {
            ...
            // Error processing sub-query list
        ...
        std::vector<app::db::Message> synthesis_context;
        ...
        for (int attempt = 0; attempt < 3 && !synthesis_success; attempt++) {
            ...
            try {
                ...
                // JSON Parsing Error (Final Synthesis)
                if (attempt == 2) {
            ...
            if (final_response_json.contains("choices") &&
                ...
                if (message.contains("tool_calls") && !message["tool_calls"].is_null()) {
                    // Warning: Final synthesis response contains tool_calls. Retrying with stronger instructions.
                    continue;
                }
            ...
            if (attempt == 2) {
                // Error: Invalid API response structure (Final Synthesis).
