...
void ForceDirectedLayout::CalculateSpringForces(const std::vector<std::shared_ptr<GraphNode>>& nodes) {
    for (const auto& node : nodes) {
        ...
        node->for_each_child([&](GraphNode* child_raw) {
            auto child_it = node_physics_.find(child_raw->shared_from_this());
...
void ForceDirectedLayout::CalculateRepulsiveForces(const std::vector<std::shared_ptr<GraphNode>>& nodes) {
    ...
    for (size_t i = 0; i < nodes.size(); ++i) {
        ...
        std::vector<int> neighbors_indices = spatial_hash_.Query(node1->position, params_.min_distance * 2);
...
void ForceDirectedLayout::CalculateTemporalForces(const std::vector<std::shared_ptr<GraphNode>>& nodes) {
    ...
    for (size_t i = 0; i < sorted_nodes.size() - 1; ++i) {
        ...
        auto it1 = node_physics_.find(node1);
        auto it2 = node_physics_.find(node2);

        if (it1 != node_physics_.end() && it2 != node_physics_.end()) {
            float y_diff = node2->position.y - node1->position.y;
            
            // Apply a force to maintain vertical order (older nodes above newer nodes)
            if (y_diff < params_.chronological_spacing * 0.8f) { // Less than ideal spacing
                float force_magnitude = params_.temporal_strength * (params_.chronological_spacing - y_diff);
            ...
            float horizontal_force = -x_diff * params_.temporal_strength * 0.1f;
