...
void MessageStore::insertMessage(const Message& msg) {
    ...
    if (sqlite3_prepare_v2(m_db_conn.getDbHandle(), sql, -1, &stmt, nullptr) != SQLITE_OK) {
         throw std::runtime_error("Failed to prepare insert statement: " + std::string(sqlite3_errmsg(m_db_conn.getDbHandle())));
    }
    ...
    if(sqlite3_step(stmt) != SQLITE_DONE) {
        throw std::runtime_error("Insert failed: " + std::string(sqlite3_errmsg(m_db_conn.getDbHandle())));
    }
}
