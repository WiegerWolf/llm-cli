...
#include "ui_interface.h" // For UserInterface
#include <thread>         // For std::async, std::launch

// Anonymous namespace for internal linkage
namespace {

// Definition for the API key retriever, used only in this translation unit.
...
// Public-facing function implementation
void chat::initialize_model_manager(
    UserInterface& ui,
    Database& db,
    std::string& active_model_id,
    ...
    model_load_future = std::async(std::launch::async, [&]() { //
        loadModelsAsync(ui, db, active_model_id, models_loading);
    });
    ...
    try {
        ...
        ui.displayStatus("Fell back to default model: " + active_model_id);
    }
    ui.setLoadingModelsState(false);
    ui.displayStatus("Model manager initialized. Active model: " + active_model_id);
    try {
...
void cacheModelsToDB(Database& db, const std::vector<ModelData>& models) {
    if (models.empty()) {
        return;
    }
    try {
...
void loadModelsAsync(
        ...
        if (!previously_selected_model_id.empty()) {
            for (const auto& model : available_models) {
                if (model.id == previously_selected_model_id) {
                    active_model_id = previously_selected_model_id;
                    model_set = true;
                    ui.displayStatus("Using previously selected model (" + context_msg + "): " + active_model_id);
                    break;
                }
            }
        }
        if (!model_set) {
            for (const auto& model : available_models) {
                if (model.id == DEFAULT_MODEL_ID) {
                    ...
                    model_set = true;
                    ui.displayStatus("Using default model ID (" + context_msg + "): " + active_model_id);
                    break;
                }
            }
        }
        if (!model_set && !available_models.empty()) {
            active_model_id = available_models[0].id;
            ui.displayStatus("Using first available model (" + context_msg + "): " + active_model_id);
        } else if (!model_set) {
            active_model_id = DEFAULT_MODEL_ID; // Fallback to compile-time default
            ui.displayError("No suitable model found in available list (" + context_msg + "). Using compile-time default: " + active_model_id);
        }
        try {
            db.saveSetting("selected_model_id", active_model_id);
        } catch (const std::exception& e) {
    ...
    try {
        ...
        cacheModelsToDB(db, fetched_models); // Throws on error
        ...
        try {
            ...
            if (cached_models.empty()) {
                ...
                try { db.saveSetting("selected_model_id", active_model_id); } catch (...) {}
