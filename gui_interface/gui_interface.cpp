...
inline std::chrono::time_point<std::chrono::system_clock, std::chrono::milliseconds>
getCurrentTimestamp() {
...
void GuiInterface::initialize() {
  ...
  // glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE); // Generally not needed unless targeting
  // very old drivers
#endif
...
GLFWwindow *GuiInterface::getWindow() const { return window; }

// --- Getters for GUI State (Used by main_gui.cpp) ---

char *GuiInterface::getInputBuffer() { return input_buf; }

size_t GuiInterface::getInputBufferSize() const { return INPUT_BUFFER_SIZE; }
...
void GuiInterface::enqueueDisplayMessage(MessageType type, const std::string &content,
                                         const std::optional<std::string> &model_id) {
  NodeIdType id = getNextMessageId();
  auto now = getCurrentTimestamp();
  std::lock_guard<std::mutex> lock(display_mutex);
  display_queue.push({id, type, content, model_id, now, kInvalidNodeId});
}
...
bool GuiInterface::isGuiMode() const { return true; }

void GuiInterface::setLoadingModelsState(bool isLoading) { models_are_loading_in_ui = isLoading; }
...
bool GuiInterface::areModelsLoadingInUI() const { return models_are_loading_in_ui.load(); }

Database &GuiInterface::getDbManager() { return db_manager_ref; }
